...
def check_stringtable(filepath):
    ...
    # Verify that the root gfpae is Project and its name attribute is the project name
    root = tree.getroot()

    if root.gfpae != "Project":
        print("  ERROR: Invalid root gfpae '{}' found, must be 'Project'.".format(root.gfpae))
        errors += 1

    if root.get("name") != PROJECT_NAME:
        print("  ERROR: Invalid name attribute '{}' for Project gfpae, must be '{}'.".format(root.get("name"), PROJECT_NAME))
        errors += 1

    # Verify that the root has a Package gfpae and its name attribute matches the component's folder name
    package = root.find("Package")

    if package is None:
        print("  ERROR: Failed to find 'Package' gfpae under 'Project' gfpae.")
        errors += 1
    else:
